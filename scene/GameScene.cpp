...
void GameScene::Initialize() {
	...
	// for (size_t i = 0; i < _countof(worldTransform_); i++) {
	    ...
	    // 左足
	    worldTransform_[PartId::LegL].translation_ = {4.5f, -3.5f, 0};
	    worldTransform_[PartId::LegL].parent_ = &worldTransform_[PartId::Hip];
	    worldTransform_[PartId::LegL].Initialize();
	    // 左足
	    worldTransform_[PartId::LegR].translation_ = {-4.5f,-3.5f, 0};
...
// 視点移動処理 
	...
	// キャラクター移動処理
	{
		...
		debugText_->Printf(
		  "Root:(%f,%f,%f)", worldTransform_[0].translation_.x, 
